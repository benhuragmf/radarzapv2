...
const ROOT = path.join(process.cwd(), 'data', 'webchat-media');

export function getWebChatMediaRoot(): string {
  return ROOT;
}

export function saveWebChatMedia(clientId: string, buffer: Buffer, ext: string): string {
...
export function resolveWebChatMediaPath(relative: string): string | null {
...
export function webChatMediaFilename(relative: string): string {
