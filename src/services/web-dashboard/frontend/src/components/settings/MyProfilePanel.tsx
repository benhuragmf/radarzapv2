...
export function MyProfilePanel({ user }: { user: AuthUser }) {
  ...
  const { data: profile, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => sessionApi.get<MemberProfile>('/auth/me/member-profile'),
  })
  ...
  const requestEmailCode = useMutation({
    mutationFn: () =>
      sessionApi.post<{ maskedEmail: string }>('/auth/me/email/request-code', {
  ...
  const confirmEmailCode = useMutation({
    mutationFn: () =>
      sessionApi.post<MemberProfile>('/auth/me/email/confirm', { email, code: emailCode }),
    onSuccess: () => {
      setEmailStep('idle')
      setEmailCode('')
  ...
  const requestWaCode = useMutation({
    mutationFn: () =>
      sessionApi.post<{ maskedPhone: string }>('/auth/me/whatsapp/request-code', { phone }),
    onSuccess: () => {
      setWaStep('code')
  ...
  const confirmWaCode = useMutation({
    mutationFn: () =>
      sessionApi.post<MemberProfile>('/auth/me/whatsapp/confirm', { phone, code: waCode }),
    onSuccess: () => {
      setWaStep('idle')
      setWaCode('')
