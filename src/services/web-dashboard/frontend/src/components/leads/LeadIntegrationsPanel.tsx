...
import { ClipboardCopy, ExternalLink, Code2, Globe } from 'lucide-react'
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--rz-text-muted)] mt-1 leading-relaxed">
              Use o <strong>formulário pronto RadarZap</strong> (recomendado) ou conecte WordPress, Elementor,
              landing pages e formulários próprios via API. Todos os envios aparecem em{' '}
              <strong>Capturas</strong> e podem virar atendimento no Inbox.
            </p>
          </div>
        </div>
      </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4">
          ...
          <p className="text-xs font-mono text-[var(--rz-text-muted)] break-all">{form.publicKey}</p>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card className="space-y-2 text-sm">
            ...
            <dl className="space-y-2 text-xs font-mono break-all">
              ...
              <div>
                <dt className="text-[var(--rz-text-muted)] font-sans">Preview local</dt>
                <dd>
                  <a
                    href={previewUrl}
                    target="_blank"
          ...
          <Card className="text-sm space-y-2">
            <p className="font-medium">Domínios permitidos</p>
            <p className="text-[var(--rz-text-muted)] text-xs leading-relaxed">
              {form.allowedDomains?.length
                ? `Somente estes hosts podem enviar: ${form.allowedDomains.join(', ')}`
                : 'Lista vazia = qualquer site pode enviar (configure em Formulários → Configurar para restringir).'}
            </p>
            <p className="text-[var(--rz-text-muted)] text-xs">
              Campos obrigatórios na API: <code>name</code>, <code>phone</code>. Opcionais:{' '}
              <code>email</code>, <code>message</code>, <code>sourceUrl</code>, <code>pageTitle</code>.
            </p>
          </Card>
        </div>
      </div>
