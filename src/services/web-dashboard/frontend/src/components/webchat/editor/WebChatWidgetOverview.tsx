...
import type { SectionStatus } from '@/lib/webchatWidgetEditorUtils'
import {
  WEBCHAT_WIDGET_EDITOR_SECTIONS,
  type WebChatWidgetEditorSectionId,
} from '../WebChatWidgetEditorSection'

type Props = {
  statuses: Record<WebChatWidgetEditorSectionId, SectionStatus>
  onNavigate: (section: WebChatWidgetEditorSectionId) => void
  editorMode: 'simple' | 'advanced'
}

const QUICK_START: WebChatWidgetEditorSectionId[] = [
  'overview',
  'visual',
  'prechat',
  'instalacao',
...
export function WebChatWidgetOverview({ statuses, onNavigate, editorMode }: Props) {
  const sections = WEBCHAT_WIDGET_EDITOR_SECTIONS.filter(
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-brand-500/20 bg-brand-500/5 p-4">
        <h4 className="text-sm font-semibold text-[var(--rz-text-primary)]">
          Por onde começar?
        </h4>
        ...
        <ol className="mt-3 space-y-2">
          {QUICK_START.filter(id => sections.some(s => s.id === id)).map((id, index) => {
            const section = WEBCHAT_WIDGET_EDITOR_SECTIONS.find(s => s.id === id)!
              ...
              <li key={id}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-medium text-[var(--rz-text-primary)]">
                      {section.label}
                    </p>
