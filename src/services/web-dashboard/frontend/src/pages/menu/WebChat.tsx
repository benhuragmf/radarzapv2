...
import { Globe, MessageSquare, Plus, Copy, Trash2, Send, XCircle } from 'lucide-react'
              ...
              {(widgets ?? []).map(w => (
                <Card key={w.id} className="p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="font-semibold text-[var(--rz-text)]">{w.name}</div>
                      <div className="mt-1 text-xs text-[var(--rz-text-muted)]">Chave: {w.publicKey}</div>
                    </div>
                    <Button
                      variant="danger"
                      size="sm"
                  ...
                  <div className="mt-1 flex gap-2">
                    <textarea className={textareaCls + ' font-mono text-xs'} readOnly rows={3} value={embedSnippet(w.publicKey)} />
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={() => {
                        navigator.clipboard.writeText(embedSnippet(w.publicKey))
