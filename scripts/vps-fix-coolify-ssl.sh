#!/usr/bin/env bash
# Garante app em :3001 + HTTPS sslip.io via Traefik Coolify; atualiza resource sem derrubar legado.
set -euo pipefail
...
log "=== 2) HTTPS Traefik → :3001 ==="
sudo bash scripts/vps-coolify-traefik-route-legacy.sh || true

log "=== 3) Atualizar Coolify (sem parar legado) ==="
export COOLIFY_COMPOSE_MODE=ghcr
export MIGRATE_LEGACY=0
sudo -E bash scripts/vps-configure-coolify-radarzap.sh || true
