/**
 * Cria produtos/preços RadarZap (Starter R$99, Pro R$299) na conta Stripe test/live.
 * Uso: npx ts-node -r dotenv/config -r tsconfig-paths/register scripts/create-stripe-radarzap-prices.ts
 */
...
async function main() {
  ...
  const plans = [
    { id: 'starter', name: 'RadarZap Starter', amount: 9900 },
    { id: 'pro', name: 'RadarZap Pro', amount: 29900 },
  ] as const;

  console.log('Criando produtos/preços RadarZap (BRL/mês)...\n');

  for (const plan of plans) {
    const product = await stripePost(
      'products',
      new URLSearchParams({ name: plan.name, 'metadata[radarzap_plan]': plan.id }),
      key,
    );
