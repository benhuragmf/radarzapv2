# RadarZap — Modos de Atendimento (Fase 5)
...
7. **LLM opcional** — se `basicTriageLlmFallbackEnabled` e confiança baixa → `completeForBasicTriage()` (chave RadarZap, tokens limitados).
...
5. Ativar toggle LLM fallback → mensagem ambígua pode consumir 1 chamada RadarZap (ver aba Limites).
