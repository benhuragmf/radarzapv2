# RadarZap — Modos de atendimento (Fase 1)

**Versão:** 2.10.106 · **Data:** 2026-06-19

Documento da primeira entrega segura da evolução de modos de atendimento. Ver análise completa em [`ANALISE-MODOS-ATENDIMENTO.md`](./ANALISE-MODOS-ATENDIMENTO.md).
...
- Tipos em `src/types/attendance-mode.ts`:
  ...
  - `AiCredentialSource` — `none` | `radarzap` | `company`
- Funções adapter:
...
O campo **`AiSettings.mode`** (`disabled` | `radarzap` | `company`) **não foi removido nem renomeado**.
...
| IA Premium + RadarZap | `radarzap` | `true` |
...
| Provedor/credencial IA | Aba Geral — radios | `mode` radarzap/company/disabled |
...
| `frontend/vite.config.ts` | Alias `@radarzap-types` |
