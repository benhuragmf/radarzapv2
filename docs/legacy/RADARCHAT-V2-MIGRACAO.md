# Radar Chat v2.0 — Notas de migração

> Cópia limpa gerada a partir de `radarchat` (jun/2026)

## Onde buscar se der erro na v2

A **v2 é a versão de trabalho**. O **v1 permanece como arquivo de referência** — não apague o `radarchat` original até a v2 estar estável em produção.

```
C:\Users\benhu\OneDrive\Área de Trabalho\Projetos\
├── radarchat\      ← v1: referência completa (histórico, lixo útil para debug, deploys antigos)
└── radarchatv2\    ← v2: código limpo e funcional (use no dia a dia)
```

### Fluxo sugerido ao investigar um problema

1. Reproduzir o erro na **v2** (`radarchatv2`).
2. Se faltar arquivo, script ou comportamento: abrir o mesmo caminho no **v1** (`radarchat`) e comparar.
...
**Stack v2 apenas** — Docker usa volumes `radarchatv2_*`. Não monte volumes do projeto `radarchat` (v1).

**Não rode** `docker compose up -d` completo (sobe `auto-setup` = segundo bot). Use só infra:

```bash
npm run docker:infra   # Redis :6380 + Mongo :27017 (volumes radarchatv2)
...
Isso faz `mongodump` do volume `radarchat_mongodb-data` (leitura única), recria `radarchatv2_mongodb-data` com a senha do `.env` e restaura. Depois disso, só volumes v2.

O v1 continua existindo na pasta `radarchat` apenas como **referência de código**, não como infraestrutura.

## Próximos passos sugeridos

1. Inicializar git em `radarchatv2` e fazer primeiro commit
