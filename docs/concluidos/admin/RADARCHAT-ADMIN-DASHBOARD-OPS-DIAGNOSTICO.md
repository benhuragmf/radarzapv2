# Radar Chat — Admin Dashboard Ops — Diagnóstico
...
**Próximo passo recomendado:** Etapa 3 — evoluir UI `/admin/dashboard` para consumir `GET /api/admin/ops/summary`. Ver [`RADARCHAT-ADMIN-DASHBOARD-OPS.md`](./RADARCHAT-ADMIN-DASHBOARD-OPS.md).
...
| Untracked (não versionados) | `data/`, `docs/RADARCHAT-PLANO-UPGRADES.md`, `docs/qa-results/`, `docs/referencias/`, `mocker/modelochat/` |
...
| **Menu** | `ADMIN_RADARCHAT_NAV` e `MODERATOR_ADMIN_NAV` — item "Dashboard global" |
| **Layout** | Aba **Admin Radar Chat** (`navConfig.ts` — `pathname.startsWith('/admin/')`) |
...
Atribuição superadmin: env `RADARCHAT_SYSTEM_ADMIN_DISCORD_IDS` (`GuildMembershipSync.ts`).
...
- **Admin plataforma:** `/admin/ai-platform/usage` — consumo keys Radar Chat (não agrega todas empresas).
...
- `docs/RADARCHAT-SISTEMA-COMPLETO.md` (estrutura geral)
- `docs/concluidos/RADARCHAT-RESULTADO-FINAL-TOP-01-20.md`
- `docs/concluidos/top/RADARCHAT-TOP-20-CONGELAMENTO-FINAL-GO-LIVE-CONTROLADO.md`
...
- `docs/WEBCHAT.md`
- `docs/EQUIPE-RBAC.md`
- `docs/concluidos/top/RADARCHAT-TOP-18-AUDITORIA-SEGURANCA-LGPD-HARDENING.md` (mascaramento)
