# Radar Chat — TOP 01/20 — Diagnóstico Inicial
...
| Modos atendimento | `RADARCHAT-MODOS-ATENDIMENTO-IMPLEMENTACAO.md` + `concluidos/PHASE-*` | Fases 1–8 concluídas |
...
| `RADARCHAT_SYSTEM_ADMIN_DISCORD_IDS` | Sim | Sim | Admin sistema | |
| `RADARCHAT_SYSTEM_MODERATOR_DISCORD_IDS` | Sim | Sim | Moderador | |
...
| `RADARCHAT_AI_OPENAI_KEY` | Comentado | Sim | IA Radar Chat | |
...
- Sessão Express + Redis (`radarchat.sid`), OAuth Google + Discord.
...
- Provedor separado: `credentialSource` radarchat vs company.
...
- Provedores: OpenAI (chave Radar Chat), Gemini (chave empresa no painel).
...
- **RISCO:** sem créditos, LLM Radar Chat bloqueia — deve cair humano/robotizado (documentado; validar E2E).
...
5. **Custo IA** — sem carteira/créditos, chamadas LLM Radar Chat devem bloquear; risco de prejuízo se gate `AiUsageMeterService` falhar.
