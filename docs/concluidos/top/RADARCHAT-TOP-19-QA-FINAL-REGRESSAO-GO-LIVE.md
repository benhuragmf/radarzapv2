# Radar Chat — TOP 19/20 — QA Final, Regressão e Checklist Pré-Go-Live
...
- **Correção aplicada:** seletores Inbox (heading no Header); radio Radar Chat com `/^Radar Chat\b/` (strict mode).
...
| E2E-02 | E2E IA modes | Low | `getByRole('radio', /Radar Chat/)` strict mode (2 elementos) | Corrigido | Seletor `/^Radar Chat\b/` |
...
4. `e2e/attendance-modes.spec.ts` — seletor radio Radar Chat.
...
- `docs/top/RADARCHAT-TOP-19-QA-FINAL-REGRESSAO-GO-LIVE.md` (novo)
- `docs/RADARCHAT-SISTEMA-COMPLETO.md` §22 e §24
...
5. Registrar resultado em template QA e fechar `RADARCHAT-TOP-20-*.md`.
