# RadarZap — TOP 06/20 — Modos de Atendimento
...
aiProvider/credential = radarzap | company | none  (AiSettings.mode legado)
...
- `radarzap`/`company` sem `attendanceMode` → `premium_assistant`.
...
**TOP 07** — conforme roteiro em `docs/top/` (consultar `RADARZAP-TOP-01-DIAGNOSTICO-INICIAL.md` para escopo da próxima etapa). Manter gates verdes; não avançar billing (TOP 17) nem recarga IA (TOP 16) antes da hora.
