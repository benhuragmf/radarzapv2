# RadarZap — TOP 16/20 — IA Créditos, Carteira, Consumo e Fallback
...
- Registro: `recordUsage` após chamada; evento `ai.credits.consumed` (RadarZap).
- Proporcional: `aiCreditsFromActualCost` (1 crédito ≈ US$0,01).
- Chave própria: débito 0 na carteira RadarZap.
...
- Premium RadarZap: `getAvailability` verifica carteira; sem crédito → mensagem segura + `shouldEscalate`.
...
LLM RadarZap debita; local/handoff/bridge/comando não debitam.
...
§19 `RADARZAP-SISTEMA-COMPLETO.md`, `IA-CREDITOS-E-CARTEIRA.md`, índice, changelog → `2.12.2`.
