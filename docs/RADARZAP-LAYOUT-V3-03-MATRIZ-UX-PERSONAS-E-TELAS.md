# RadarZap Layout v3 03 — Matriz UX por Personas e Telas

## 1. Objetivo

Garantir que o novo visual do RadarZap seja pensado para cliente final, atendente, supervisor, dono da empresa, admin SaaS e dono/desenvolvedor do RadarZap. Esta matriz não altera telas; ela define perguntas, ações e riscos para orientar as próximas fases.
...
| Dono/desenvolvedor RadarZap | Rotas, permissões, QA e evolução segura | Executar fases, preservar contratos, documentar evidência | Refatoração ampla sem inventário |
...
|------|------|-------------------|--------------------------------------|----------------|-------------------------|---------------|-------------------|----------------|
| Login | `/` sem sessão | Todos | Estou entrando no RadarZap certo? | Entrar | Marca, erro de login | Jargão interno | "Painel Administrativo" para cliente | Copy SaaS mais clara |
...
| Admin SaaS | `/admin/dashboard` | Staff RadarZap | O SaaS está saudável? | Investigar empresa/falha | Filas, erros, billing, auditoria | Dados tenant sem contexto | Global x tenant | Marcadores visuais fortes |
